...
def get_pipeline():
    ...
    checkpoint = os.environ.get("WAV2VEC2_CHECKPOINT")
    if not checkpoint:
        raise ValueError("Set WAV2VEC2_CHECKPOINT to a saved Persian CTC model directory or Hub ID")
...
async def analyse(
    ...
    try:
        ...
        pipeline = await run_in_threadpool(get_pipeline) if do_asr else None
