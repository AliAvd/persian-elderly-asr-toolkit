...
ROOT=Path(os.environ.get('ASR_DATA_ROOT', str(Path(__file__).resolve().parents[1]))).expanduser().resolve();HF=ROOT/'Final_Gathered_Dataset_HF';REPO='AliAvd/persian-elderly-asr'
def main():
 api=HfApi();info=api.repo_info(REPO,repo_type='dataset');assert info.private;revision=info.sha
 files={r.rfilename for r in info.siblings};expected={s:v['samples'] for s,v in json.loads((HF/'processing_summary.json').read_text())['splits'].items()}
